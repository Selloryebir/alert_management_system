...
- AC-022 绑定候选后，候选到发布树只允许修改 `automation/state.json`、M14 证据、README、来源矩阵、六份项目报告及其对应 DOCX/PDF 和清单；源码、脚本、配置、样例、手册或其他文件发生变化必须重建 ZIP 并重新人工终验。
- `candidate`、`approved`、`post-main`、`released` 四种模式都查询远端 `v1.0.0` 并要求其不存在；候选和批准阶段还要求本地、远端 `v1.0.0-rc.1` 不存在，避免旧标签或提前发布绕过受审树。
