...
M14 把已经通过 M13 的产品、手册和发布链冻结为 `v1.0.0` 正式发布候选。Windows 11 x64 自包含 ZIP 是首要路径；HTTPS Docker Compose 是次级部署路径。两者复用同一 Java、Python、PostgreSQL、Vue 实现和工程场景数据，不增加安装器、Windows 服务、网关或第二套业务逻辑。

自动化证明构件完整、命令可执行、业务闭环与失败边界成立。人工层可以采用组织逐项验收单，也可以采用项目负责人的结果性声明；采用后者时只记录声明原文、固定候选、已验证 ZIP 摘要、记录时间和发布授权，不推断未提供的身份、环境或签名。

## 2. 最小实施范围

### 2.1 Windows 发布候选预验收

- `scripts/release/verify-business-release.ps1` 作为唯一 M14 Windows 自动入口，拒绝脏工作区，固定版本 `1.0.0`，在独立输出目录构建并验收 ZIP。
...
- `scripts/validate_release_candidate.py` 检查 M0–M13 状态、57 项来源处置、M13 事实收口、正式交付物摘要、版本入口、Git 工作树、人工声明和标签边界。
- 人工声明绑定候选后，只允许修改说明书、项目报告、专利草案、发布治理、版本元数据及对应 DOCX/PDF；业务源码、迁移、算法、样例或用户可见流程发生变化必须重建 ZIP 并重新验收。
- `candidate` 和 `approved` 模式要求本地、远端 `v1.0.0` 不存在；`post-main` 与 `released` 要求 annotated `v1.0.0`、远端 main 和 dev 可达记录的发布提交。
...
自动门槛与独立复审通过后，人工审查至少取得以下材料：

- 固定候选 ZIP；
- 同名 `.sha256`；
- ZIP 内 `manuals` 的业务手册和部署手册。

若组织采用逐项验收单，应按随包手册完成哈希核对、全新解压、预检、启动、网页登录与首次改密、项目与 300 行样例闭环、PDF/XLSX、备份与隔离恢复及停止。若项目负责人已经实际审查并作出结果性判断，可在 `docs/verification/evidence/M14.md` 保存其原始声明，并在 `automation/state.json` 绑定候选提交、已验证 ZIP SHA-256、记录时间、PASS 和正式发布授权。发布校验器同时核对两处，不能仅靠把阶段状态改成 `passed` 绕过人工决策。

人工结果未返回、未明确授权正式发布或发现影响交付的重大问题时，M14 使用 `awaiting_human` 或回到定向修复，不标记 `passed`，不创建正式标签。

## 5. Git 与发布顺序

1. 在 `feat/m14-business-release` 形成固定实现候选，完成三条自动门槛和独立复审。
2. 以该候选构建 ZIP，完成人工业务验收。
...
6. 证据完整且标签不存在时在已验证的 main 提交创建 annotated `v1.0.0`；标签事件显式运行 `--mode post-main`，并验证标签、远端 main 和 dev 均可达同一发布结果。
7. M14 最终状态和证据同步回 `dev` 后运行 `--mode released`，完成技术发布闭环。
