...
本文定义下一版报警分析算法 `hybrid-v2` 的最小实施基线，覆盖模型边界、数学定义、参数、解释义务、泛化验收、性能目标和未来训练门槛。它是实现与审查契约，不是当前代码已经达到的验收结论。

建议版本标识如下：

| 项目 | 版本 | 说明 |
| --- | --- | --- |
| HTTP JSON 契约 | `v1` | 响应字段形状不变时继续使用；新增或改变必填字段语义时必须升版 |
...
当前 `rules-v1.0.0` 不是按记录 ID、源行号或完整输入行返回固定结果的直接查表。重复、抖动、短时恢复和持续报警会依据输入记录的时间、状态和字段执行确定性规则，能够处理未见过的 UUID、时间和位号，因此这部分属于可解释专家规则，可以作为 v2 的基础。

但当前整体实现尚不满足“真实可泛化模型”的要求：

- 每类报警使用固定分数，不随时间距离、状态转换或证据强弱变化，不能解释为概率或经过校准的置信度。
- 原因类别依赖代码中的固定子串词表，没有显式特征权重、冲突处理、弃权门槛、参数来源或独立验证。
- 事件链依赖位号或描述中的固定场景名称和“步骤 1..5”，而合成样例生成器会产生相同标记；这只能验证预编排场景，不能证明发现未见关系的能力。
- 时间先后命中的事件链会直接影响原因类别，容易把相关性误写为因果关系。
- 现有固定样例与预期可用于回归，但不能代替独立真实数据上的准确率、泛化或根因证据。

因此，v2 必须删除运行时代码对合成场景名、固定步骤、固定 UUID、源行区间和预期结果文件的依赖，同时保留 v1 已经成立的纯计算、确定性、解释性和 Java 最终写权限边界。
...
三个部分都必须只读取请求和冻结参数。服务不得在线修改模型文件、保留跨请求隐式状态或访问 PostgreSQL。
...
若 `return_time` 可用，解释中同时给出报警持续时间 `d_i` 和相邻恢复间隔；不得因缺少 `return_time` 人为补造持续时间。该定义使用报警/非报警转换而不是任意状态字符串变化，避免把确认动作误判为抖动。
...
实现时如将权重外置，只允许一个只读、版本化的模型文件，例如 `expert-model-v2.json`。该文件至少记录：
...
模型文件不得在服务运行时训练或改写。未经过人工审核的词条只能停留在候选材料中，不能直接进入发布模型。
...
- 抖动：窗口起止、记录数、二值状态序列、转换次数、转换比和阈值；
...
- 时序窗口扫描：使用双指针和按组状态，时间 `O(n)`，不得对同组记录做无界全对全比较；
...
- 原因专家模型只覆盖已审查特征；新厂商词汇、缩写、否定表达和不同语言可能返回 `UNKNOWN`。
...
只有同时满足以下条件，才允许引入线性 SVM、浅层决策树或其他监督模型：
...
10. 未通过独立真实测试和人工门槛前，只能标记为实验候选，不替换 `hybrid-v2`。

真实数据量不足时，不应为了满足“机器学习”名称而用合成数据训练 SVM/树。合成数据适合验证算法可达性、确定性和负控，不适合证明现场准确率。
