...
本文定义并记录报警分析算法 `hybrid-v2` 的实施基线，覆盖模型边界、数学定义、参数、解释义务、泛化验收、性能目标和未来训练门槛。实现只有在 M9 验收证据绑定固定提交后才视为阶段通过。
...
`hybrid-v2.0.0` 已在 M9 候选中实现：规则强度随时间距离或状态转换证据计算；原因类别由非负权重、分数门槛和类别间距决定并允许弃权；关联链由同一关系范围内跨 episode 的一阶 Markov 统计发现。运行时代码不读取合成场景名、固定步骤、固定 UUID、源行区间或预期结果文件，Python 保持纯计算，Java 保持最终写权限。

固定样例与独立金样只证明实现符合已声明场景；未见数据与变形黑盒证明算法不依赖演示标识。两者都不能代替脱敏真实标注集上的准确率、因果结论或长期现场性能。
