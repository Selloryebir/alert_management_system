...
### 2.1 第一次打开 PowerShell

本手册中的命令都在 PowerShell 中执行。办公室人员可按以下步骤打开：

1. 点击屏幕底部的 Windows“开始”按钮，或按键盘上的 Windows 徽标键。
2. 直接输入 `PowerShell`。
3. 点击“Windows PowerShell”或“终端”。普通原生部署不需要选择“以管理员身份运行”。
4. 看到以 `PS C:\` 开头的窗口后再复制命令；每次只复制一个完整代码框，按 Enter 执行并等待结果。
