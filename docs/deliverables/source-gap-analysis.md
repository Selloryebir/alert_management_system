...
| 字段 | 内容 |
|---|---|
| 文档性质 | 原始 PDF/DOCX 能力与当前正式产品之间的逐项责任说明 |
| 产品名称 | 报警管理系统 |
| 编制日期 | 2026-08-27 |
| 当前技术基线 | `v0.8.0`、M9–M12 固定证据及 M13 当前阶段 |
| 项目编号 | 由项目管理方填写 |
| 原始材料中的人员、单位、预算、审批与签章 | 不继承；由项目管理方依据原始凭证填写，不在技术仓库证明范围 |

## 2. 证据基线、方法与范围

本说明以[历史来源能力闭环矩阵](../product/source-coverage.md)的 57 项能力为全集，以[需求追踪](../product/requirements.md)、[系统架构](../architecture/README.md)、[M7](../verification/evidence/M7.md)和 [M9](../verification/evidence/M9.md)至 [M12](../verification/evidence/M12.md)为当前事实依据。

原始材料处于早期编撰状态，允许存在冲突、夸张、跨项目内容和占位代码。本说明不删除原文，而是把其能力分为：已实现、M13、M14、外部条件后重启、明确拒绝。每项只有一个当前责任；“外部条件后重启”不是排期或承诺，“明确拒绝”不得通过隐藏配置恢复。

当前产品只证明 Windows 11 x64、固定合成数据和有限运行窗口下的结果。真实性能、准确率、长期运行、合规和外部集成必须有新的责任人、数据、环境与验收协议。

## 3. 总体差距结论

当前方案比原始方案更适合交付，原因不是功能堆叠更多，而是每项能力均有可达实现或明确停止条件：
...
| ID | 原始能力 | 唯一处置 | 当前响应、证据与理由 |
|---|---|---|---|
| SC-001 | 正式产品名称与版本 | 已实现 | 页面、健康响应、报告、README、运行包和发布标签统一使用“报警管理系统”；历史研发叙述只保留在来源与证据区。 |
| SC-002 | 原始资料、占位代码和无关内容隔离 | 已实现 | 四份原件及提取清单保留，PDF 占位代码不进入源码或实现提示，无关项目只登记来源。 |
...
| SC-015 | 饼图、折线、排名、热力图、缩放和导出 | 已实现（调整） | 提供趋势、分布、区域/装置排名、打印和 CSV；缺少坐标与拓扑，拒绝伪造热力图和地图缩放语义。 |
...
| SC-022 | Word 报告和可编辑正式交付文档 | M13 | 本阶段以 Markdown 单一事实源生成 DOCX/PDF 项目文档；业务数据报告仍为 PDF/XLSX，M13 门槛通过前不记为完成。 |
...
| SC-040 | 正式业务、部署、恢复和故障说明 | M13 | 本阶段从最终实现事实编制中文手册和随包说明；文档构建、链接与操作可达性通过 M13 门槛后完成。 |
| SC-041 | 立项、中期、测试、结项和过程记录 | M13 | 本目录以当前 Git 和证据重写正式过程文件，不复制历史“已完成”陈述；DOCX/PDF 与 Markdown 一致后通过。 |
...
本说明完成来源能力的技术责任解释，但不删除或改写原始档案，也不证明原始行政信息真实。M13 门槛通过后，过程文档才成为正式交付产物；M14 和有权限人员审批完成后，项目管理方才能作出最终验收与结项决定。
