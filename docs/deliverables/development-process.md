...
| 字段 | 内容 |
|---|---|
| 文档性质 | 基于 Git、自动化状态与阶段证据重建的技术开发过程事实源 |
| 产品名称 | 报警管理系统 |
| 记录范围 | M0–M12 已完成事实及 M13–M14 当前责任 |
| 编制日期 | 2026-08-27 |
| 项目编号 | 由项目管理方填写 |
...
本记录以提交对象、分支、PR、标签、自动化状态、实际命令输出和 `docs/verification/evidence/` 为证据。它描述责任域和技术活动，不虚构个人姓名、组织人数、工时、会议、采购、预算或行政审批。

当前正式发布标签为 `v0.8.0`。M12 后 `dev` 已解锁 M13；M13 文档与 M14 用户终验不能因写入计划而提前记为通过。
...
| 阶段 | 主要活动 | 形成的检查点或证据 |
|---|---|---|
| M0 | 保存四份原件，提取 Markdown/图片，登记 manifest，隔离占位代码 | 来源目录、清单和提取边界 |
...
| M13 | 形成说明书和正式项目过程文档 | 当前阶段，未通过前不得写成完成 |
| M14 | Windows 11 非技术业务用户终验和最终候选冻结 | 当前阻塞于 M13，需人工终验 |
...
M0–M12 已有阶段证据，当前执行 M13。M13 必须让 Markdown、DOCX、PDF、说明书、源码和运行行为一致；M14 必须由不熟悉代码的业务人员按说明完成部署及完整业务闭环。正式发布或结项仍需人工授权。
