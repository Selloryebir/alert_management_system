...
本仓库提供报警文件导入、规范化、可解释分析、统计看板、人工处置、审计、报告和备份恢复验证的一体化应用。`v0.8.0` 已通过 Windows 11 x64 自包含运行包、Docker Compose、身份权限、HTTPS 边界和有限可靠性验收；M14 固定候选已通过自动验收并获得项目负责人交付可行性确认，发布目标为 `v1.0.0`。
...
`v0.8.0` 精确指向 `main` 的安全部署与可靠性发布提交。M14 固定候选的源码、Windows ZIP、SHA-256、说明书和部署验收已有绑定证据；项目负责人已确认符合交付预期并授权正式发布。`v1.0.0` 仍只在发布 PR、远端 CI、main/dev 同步和 annotated tag 条件全部满足后创建。版本标签和发布门槛见 [`docs/releases/versioning.md`](docs/releases/versioning.md)。
