...
本仓库用于重建报警管理系统的可演示版本。M1 已建立 Java、Python、Vue 和 PostgreSQL 的可运行工程骨架；M2 已实现合成样例及文件导入闭环；M3 已实现可解释规则分析，处置和报告按后续里程碑实现。
...
M0、M1、M2、M3 已通过；M4 前端业务闭环已解锁。状态以 `automation/state.json` 及对应阶段的当前提交证据为准。
