...
async function confirmInvalidation() {
  ...
  if (!invalidateReason.value.trim()) {
    errorMessage.value = "请填写作废理由。";
    return;
  }
  busy.value = true;
  errorMessage.value = "";
  try {
    await invalidateManualAlarm(props.projectId, invalidating.value.record_id, invalidateReason.value.trim());
    invalidating.value = undefined;
    await loadAlarms();
...
<template>
  <section class="manual-panel" data-testid="manual-alarm" aria-labelledby="manual-title">
    ...
    <form v-if="showCreate || editing" class="manual-form" @submit.prevent="editing ? saveEdit() : saveCreate()">
      ...
      <label>来源系统（必填）<input v-model="form.source_system" required /></label><label>源操作员（必填）<input v-model="form.operator" required /></label>
      ...
      <template v-if="editing"><label class="wide-field">修订理由（必填）<input v-model="editReason" required /></label></template>
    ...
    <form v-if="invalidating" class="invalidation-form" @submit.prevent="confirmInvalidation"><h4>确认作废 {{ invalidating.tag }}</h4><p class="empty-copy">操作身份取自当前登录账号。</p><label>作废理由（必填）<input v-model="invalidateReason" required /></label><button type="submit" class="danger-button" :disabled="busy">确认作废</button></form>
  </section>
</template>
